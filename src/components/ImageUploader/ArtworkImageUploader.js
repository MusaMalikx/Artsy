...
const uploadImage = (event, setIsUploaded) => {
  ...
  reader.addEventListener('load', () => {
    ...
    setIsUploaded((prev) => {
      if (prev.indexOf(uploaded_image) === -1) return [...prev, uploaded_image];
      else return prev;
    });
  });
  ...
  return (
    <>
      {isUploaded.map((pic, i) => {
        return (
          <div key={i} className="border w-32 h-32">
            <label
              htmlFor={`img-${isUploaded.indexOf(pic)}`}
              className=" flex text-lg text-blue-500 hover:cursor-pointer justify-center items-center h-full ">
              <img className="cursor-pointer w-32 h-32" src={pic} alt="" />
            </label>
            <input
...
export default function ArtworkImageUploader() {
  ...
  return (
    <div className="w-full flex justify-evenly p-5 flex-wrap max-w-md ml-2 my-3 border">
      {isUploaded.length === 0
        ? displayDefaultUpload(isUploaded, setIsUploaded)
        : displayUploadedImages(isUploaded, setIsUploaded)}
    </div>
  );
}
